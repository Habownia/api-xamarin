...
namespace api_xamarin
{
    ...
    public class ApiHelper
    {
        ...
        public async Task<ExchangeRate> GetApiDataAsync()
        {
            try
            {
                List<ExchangeRate> fullExchangeRate = new List<ExchangeRate>();
                string[] letters = { "A", "B"};
                foreach (string letter in letters)
                {
                    var response = await _httpClient.GetStringAsync(ApiBaseUrl + "exchangerates/tables/" + letter + "//?format=json"); // endpoint do api tabla A, a jest ich 3 - A,B,C, c to jest bid ask
                    ...
                    if (!string.IsNullOrEmpty(response))
                    {
                        ...
                        if(fullExchangeRate.Count <= 0)
                        {
                            fullExchangeRate.AddRange(exchangeRates);
                        }
                        else
                        {
                            fullExchangeRate[0].rates.AddRange(exchangeRates[0].rates);
                        }
                    }
                }
                return fullExchangeRate[0];
            }
            catch (Exception ex)
            {
